...
export default function EmailCampaign() {
	...
	const [AllEmailCampaignData, setAllEmailCampaignData] = useState("");
	...
	useEffect(() => {
		if (id) getAllEmailCampaign(option, status, id);
	}, [id]);

	const handleStatusWise = (e) => {
		setstatus(e.target.value);
		getAllEmailCampaign(option, e.target.value, id);
	};

	const getAllEmailCampaign = (option, status, id) => {
		let data = { status: status ? status : "all", id: id, option: option };
		const getAllEmailCampaignPromise = new Promise((resolve, reject) => resolve(PostApi(API_Path.getEmailcampaign, data)));
		getAllEmailCampaignPromise.then((res) => {
			if (res.status === 200) {
				// if (res.data?.data.length > 0) {
				setAllEmailCampaignData(res.data.data.emailcampign);
				setoption({ ...option, totalRecord: res.data.data.totalRecord });
				// } else {
				// 	setAllEmailCampaignData([]);
	...
	const getstatuswiseData = () => {
		const getstatuswiseDataPromise = new Promise((resolve, reject) => {
			resolve(PostApi(API_Path.export_emailcampaign, { status: status }));
		});
	...
	const tableCallBack = (option) => {
		setoption(option);
		getAllEmailCampaign(option, status, id);
	};
	...
	const deleteEmailCampaign = () => {
		const deleteEmailCampaignPromise = new Promise((resolve, reject) => {
			resolve(PostApi(API_Path.deleteEmailcampaign, { id: id }));
		});
		deleteEmailCampaignPromise.then((res) => {
			if (res.status === 200) {
	...
	const handleChangeStatus = (id, status) => {
		...
		const changeStatusEmailCampaignPromise = new Promise((resolve, reject) => {
			resolve(PostApi(API_Path.changeEmailcampaignStatus, data));
		});
		changeStatusEmailCampaignPromise.then((res) => {
			if (res.status === 200) {
				toast.success("Status updated successfully.");
				getAllEmailCampaign(option, status, context.login_user_id);
			} else {
				toast.error(res.data.message);
			}
		});
	...
	const columns = [
		...
		},
		{
			...
			options: {
				...
				sort: false,
				customBodyRender: (data, i) => {
					return <div className={`status-col status-${data[i].status === "sent" ? "active" : data[i].status === "scheduled" ? "pending" : "rejected"}`}>{data[i].status}</div>;
				},
			},
		},
		{
					...
					return (
						<div className="table-ed-drop">
							<Dropdown drop="left">
								...
								<Dropdown.Menu>
									<Dropdown.Item onClick={() => Navigate("/email-campaign-detail/", { state: { id: data[i]._id } })}>
									...
									{data[i].status !== "sent" && (
										<Dropdown.Item onClick={() => Navigate("/edit-email-campaign/", { state: { id: data[i]._id } })}>
											<bdi className="d-flex align-items-center">
					...
					<div className="row align-items-center">
						<div className="col-md-4 ">
							<div className="comn-title-main">
								<h1 className="mb-0">Email Campaigns</h1>
							</div>
						</div>
						<div className="col-md-8  text-camp-rgt">
							...
							<div className="crt-campaign-btn crt-in-btn  my-2">
								<button className="btn-comn-class" onClick={() => Navigate("/create-email-campaign")}>
									<span>Create A Campaign</span>
								</button>
							</div>
							<div className="crt-campaign-btn crt-in-btn  my-2" onClick={() => getstatuswiseData()}>
								<a className="btn-comn-class w-100" id="csv" href={AllEmailCampaignData !== undefined && `data:text/csv;charset=utf-8,${escape(DataInCSV)}`} download="EmailCampaignList.csv">Export</a>
							</div>
						</div>
						<div className="col-12">
							<div className="bx-white-main">
								<RtdDatatable option={option} columns={columns} data={AllEmailCampaignData} tableCallBack={tableCallBack} />
							</div>
						</div>
					</div>
					{delete_modal_show && (
						<Modal dialogClassName="modal-dialog modal-dialog-centered modal-cust-main-cmn delted-modal" show={delete_modal_show} onHide={delete_handleClose}>
							...
							<Modal.Body>
								<div className="text-center dltd-text-info">
									...
									<div className="row">
										<div className="col-6">
											<button type="button" className="btn-comn-class w-100" onClick={deleteEmailCampaign}>
												Yes
											</button>
										</div>
										<div className="col-6">
